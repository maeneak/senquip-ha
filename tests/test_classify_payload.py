...
class TestClassifyInternal:
    ...
    def test_known_internal_sensors_discovered(self):
        result, _ = _classify_payload(EXAMPLE_PAYLOAD)
    ...
    def test_unknown_internal_uses_title_case(self):
        ...
        result, _ = _classify_payload(payload)
        internal = result["Internal"]
...
class TestClassifyEdgeCases:
    ...
    def test_empty_payload(self):
        result, _ = _classify_payload({})
        assert result == {}

    def test_only_metadata(self):
        result, _ = _classify_payload({"deviceid": "X", "ts": 1, "time": 2})
        assert result == {}

    def test_empty_can_list(self):
        result, _ = _classify_payload({"can1": []})
        # Option 3: DM1 sensors always included even with no frames
        assert "CAN1" in result
        can1_sensors = result["CAN1"]
        # Should only have DM1 sensors, no regular SPNs
        assert all(sensor.key.startswith("can1.dm1.") for sensor in can1_sensors)
        assert len(can1_sensors) == 8  # 8 DM1 sensors

    def test_missing_frame_fields(self):
        result, _ = _classify_payload({"can1": [{"id": 1}, {"data": "AA"}, {}]})
        # Option 3: DM1 sensors always included even with no valid frames
        assert "CAN1" in result
        can1_sensors = result["CAN1"]
        # Should only have DM1 sensors, no regular SPNs
        assert all(sensor.key.startswith("can1.dm1.") for sensor in can1_sensors)
    ...
    def test_empty_events_list(self):
        result, _ = _classify_payload({"events": []})
        events = result.get("Events", [])
        assert len(events) == 1
    ...
    def test_non_numeric_non_str_values_ignored(self):
        ...
        result, _ = _classify_payload({"nested": {"a": 1}})
